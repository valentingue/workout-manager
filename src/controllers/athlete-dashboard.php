...
foreach($posts as $k => $post){
    ...
    foreach($post->acf_fields['workout_field_exercices'] as $i => $exercice){
        $posts[$k]->acf_fields['workout_field_exercices'][$i]['exercice_image'] = wp_get_attachment_image_url($exercice['exercice_image']);
    }
    
    // if workout start date > today && end date < today => add workout to current workouts array
    if ( date('Y-m-d', strtotime($post->acf_fields['workout_field_start_date'])) < date('Y-m-d') && date('Y-m-d', strtotime($post->acf_fields['workout_field_end_date'])) > date('Y-m-d')){

        $dates = dateRange($post->acf_fields['workout_field_start_date'], $post->acf_fields['workout_field_end_date']);
        
        $days = []; 
        $training_days = [];

        foreach($post->acf_fields['workout_field_training_days'] as $k => $training_day){
    ...
    if ($get_archived_workout){
        ...
        if (strtotime($post->acf_fields['workout_field_end_date']) < strtotime(date('Y-m-d'))) $context["archived_workout"][] = $post; 
    }
    ...
    if ($get_future_workout){
        ...
        if ( strtotime($post->acf_fields['workout_field_start_date']) > strtotime(date('Y-m-d')) && strtotime($post->acf_fields['workout_field_end_date']) > strtotime(date('Y-m-d'))){
...
$view_path  = WORKOUT_MANAGER_DIR."/includes/templates/dashboard.twig";
